...
```{r load_str_data, message=FALSE, warning=FALSE, comment=FALSE, cache=TRUE, echo=TRUE}
  ...
  str_df <- readRDS(file.path(data_path, 'prepared', 'st_prop.RDS'))

  # Daily rental information

  daily_df <- readRDS(file.path(data_path, 'prepared', 'st_daily.RDS'))
...
```{r load_ltr_data, message=FALSE, warning=FALSE, comment=FALSE, cache=TRUE, echo=TRUE}
 ...
 # Property level long term ltral information
  ...
  ltr_df <- readRDS(file.path(data_path, 'prepared', 'lt_prop.RDS'))

 # Long term ltral listing information

  list_df <- readRDS(file.path(data_path, 'prepared', 'lt_list.RDS'))
...
```{r load_sp_data, message=FALSE, warning=FALSE, comment=FALSE, cache=TRUE, echo=TRUE}
  ...
  suburbs_shp <- readShapePoly(file.path(data_path, 'geographic', 'melbSuburbs.shp'),
                               proj4string=CRS("+init=epsg:4283"),
                               delete_null_obj=TRUE)
  names(suburbs_shp@data) <- tolower(names(suburbs_shp@data))

  # SA1 shapefile

  sa1s_shp <- readShapePoly(file.path(data_path, 'geographic', 'sa1s.shp'),
                            proj4string=CRS("+init=epsg:4283"),
                            delete_null_obj=TRUE)
  names(sa1s_shp@data) <- tolower(names(sa1s_shp@data))

  # Beach file

  beach_shp <- readShapePoly(file.path(data_path, 'geographic','portPhillipBeach.shp'),
                             proj4string=CRS("+init=epsg:4283"),
                             delete_null_obj=TRUE)
  names(beach_shp@data) <- tolower(names(beach_shp@data))
...
While trimming the fields we also we standardize the field names.  Here we follow the convention of placing an underscore between words in the name.  We also shorten a number of the fields to make future coding easier.  
...
```{r trim_st, message=FALSE, warning=FALSE, comment=FALSE, cache=TRUE, echo=TRUE}
   ...
   str_df <- str_df %>% 
     dplyr::select(property_id=property.id, host_id=host.id, listing_title=listing.title, 
                   ...
                   created_date=created.date, latitude, longitude, rating=overall.rating,
                   max_guests=max.guests, canc_policy=cancellation.policy, 
                   cleaning_fee=cleaning.fee, nightly_rate=published.nightly.rate, 
                   weekly_rate=published.weekly.rate, monthly_rate=published.monthly.rate, 
                   avg_nightly_rate=average.daily.rate, min_stay=minimum.stay, bedrooms, 
                   bathrooms, min_date=min.date, max_date=max.date, days=total.days, 
                   block_rate=block.rate, avail_rate=avail.rate, 
                   longest_block=longest.block, nbr_blocks=nbr.block, med_block=med.block,
                   created_year=created.year)

  daily_df <- daily_df %>%
    dplyr::select(property_id=property.id, date, status, price, booked_date=booked.date,
...
```{r trim_lt, message=FALSE, warning=FALSE, comment=FALSE, cache=TRUE, echo=TRUE}
  ...
  ltr_df <- ltr_df %>%
    dplyr::select(id_key=id.key, geo_id=geographicalid, event_id=eventid, 
                  ...
                  str_longitude=street_centroid_longitude, event_date=event.date,
                  event_price=eventprice, property_type=propertytype, area_size=areasize,
                  bedrooms, baths, parking, has_study=hasstudy, 
                  has_courtyard=hascourtyard, has_balcony=hasbalcony, 
                  has_ac=hasairconditioning, has_garage=hasgarage, dom)

  list_df <- list_df %>%
    dplyr::select(id_key=id.key, event_id=eventid, activity_id=activityid, 
                  addr_id=addressid, of_record=of.record, event_date=event.date,
                  event_price=eventprice, first_date=first.adv.date, 
                  first_price=firstadvertisedeventprice, last_date=last.adv.date,
                  last_price=lastadvertisedeventprice, event_code=eventtypecode)
...
#   
#   str_df <- str_df[(str_df$max.date - str_df$min.date) >= 178, ]
#   
#   str_df <- str_df[str_df$max.date >= (strike_date + 182) &
#                          str_df$min.date <= (strike_date + 182), ]
#  
#   daily_df <- daily_df[daily_df$date >= strike_date, ]
# 
#  # Limit long term data  
#   
#   ltr_df <- ltr_df[ltr_df$event.date >= (strike_date - 365) & 
#                             ltr_df$event.date <= (strike_date - 1), ]
#   listing.data <- listing.data[listing.data$id.key %in% ltr_df$id.key, ]
#    
#  # Count cleaning
#   
#   countCleaning('time')
...
# 
#   str_df$str.month <- 1
#   daily_df$str.month <- as.numeric(as.factor(substr(daily_df$date, 1, 7)))
# 
#   # Long term data
# 
#   ltr_df$ltr.month <- as.numeric(as.factor(substr(ltr_df$event.date, 1, 7)))
...
# 
#   str_df$type <- 'Other'
#   str_df$type[str_df$property.type %in% c('Apartment', 'Condominium')] <- 'Apartment'
#   str_df$type[str_df$property.type %in% c('House', 'Townhouse')] <- 'House'
# 
#  # Long Term data  
# 
#   ltr_df$type <- 'Other'
#   ltr_df$type[ltr_df$property.type %in% c('Duplex', 'House',
#                                            'Terrace', 'Townhouse')] <- 'House'
#   ltr_df$type[ltr_df$property.type %in% c('Studio', 'Unit')] <- 'Apartment'
# 
# ```
# 
...
#   # Create Airbnb plot
# 
#   str.types <- ggplot(str_df, aes(x=type)) + 
...
# 
#   ltr.types <- ggplot(ltr_df, aes(x=type)) + 
...
# 
#   str_df <- subset(str_df, type != 'Other')
#   ltr_df <- subset(ltr_df, type != 'Other')
#   
#   # Count Cleaning
...
#   str_df$listing.type <- as.character(str_df$listing.type)
#   str_df$listing.type[str_df$listing.type == 'Entire home/apt'] <- 'Entire home'
# 
#   list.types <- ggplot(str_df, aes(x=listing.type)) + 
#       geom_bar(color=str_col, fill=str_col) +
#       xlab('\nListing Type\n') + 
...
# 
#   str_df <- str_df[str_df$listing.type == 'Entire home', ]
#   
#   # Count cleaning 
...
# 
#   ltr_df$has.study <- ifelse(ltr_df$has.study == 'True', 1, 0)
#   ltr_df$has.courtyard <- ifelse(ltr_df$has.courtyard == 'True', 1, 0)
#   ltr_df$has.aircon <- ifelse(ltr_df$has.aircon == 'True', 1, 0)
#   ltr_df$has.balcony <- ifelse(ltr_df$has.balcony == 'True', 1, 0)
#   ltr_df$has.garage <- ifelse(ltr_df$has.garage == 'True', 1, 0)
# 
# ```
# 
# &nbsp;
...
# 
#   # Long term data
#   ltr_df <- ltr_df[!is.na(ltr_df$bedrooms), ]
#   ltr_df <- ltr_df[ltr_df$bedrooms <= 10, ]
# 
#   # Short term data
#   str_df <- str_df[!is.na(str_df$bedrooms), ]
#   str_df <- str_df[str_df$bedrooms <= 10, ]
# 
# ```
# 
...
# 
#    ltrbed <- ggplot(ltr_df, aes(x=bedrooms)) + 
...
# 
#   strbed <- ggplot(str_df, aes(x=bedrooms)) + 
...
# 
#   str_df <- str_df[str_df$bedrooms < 5, ]
#   ltr_df <- ltr_df[ltr_df$bedrooms < 5, ]
# 
#  # Remove apartments with more than 3
#   
#   str.id <- which(str_df$bedrooms > 3 & str_df$type =='Apartment')
#   ltr.id <- which(ltr_df$bedrooms > 3 & ltr_df$type =='Apartment')
# 
#   if(length(str.id) > 0) str_df <- str_df[-str.id, ]
#   if(length(ltr.id) > 0) ltr_df <- ltr_df[-ltr.id, ]
# 
#   # Convert 0 Bed to 1 Bed  
#   str_df$bedrooms[str_df$bedrooms == 0] <- 1
#   
#   # Count Data Cleaning
...
#  # Remove missing data
#   
#   ltr_df <- ltr_df[!is.na(ltr_df$baths), ]
#   str_df <- str_df[!is.na(str_df$baths), ]
# 
# ```
# 
...
# 
#   ltbath <- ggplot(ltr_df, aes(x=baths)) + 
#     geom_bar(fill=ltr_col, color=ltr_col) +
#     facet_wrap(~type) +
...
# 
#   stbath <- ggplot(str_df, aes(x=baths)) + 
#     geom_bar(fill=str_col, color=str_col) +
#     facet_wrap(~type, scales='free_y') +
...
# 
#   # Short term data
#   str_df <- str_df[str_df$baths < 4, ]
#   str_df <- str_df[str_df$baths > 0, ]
# 
#   # Long term data
#   ltr_df <- ltr_df[ltr_df$baths < 4, ]
#   
#  # Round baths up  
# 
#   str_df$baths <- round(str_df$baths + .01, 0)
# 
#   # Count cleaning
#   countCleaning('baths')
#   
...
# 
#   ltr_df$bedbath <- paste0(ltr_df$bedrooms, '..', ltr_df$baths)
#   str_df$bedbath <- paste0(str_df$bedrooms, '..', str_df$baths)
# 
# ```
# 
...
#  # Make long term plot
# 
#   ltbb <- ggplot(ltr_df, aes(x=bedbath)) + 
#     geom_bar(fill=ltr_col, color=ltr_col) +
#     xlab('\nBed/Bath Combo\n') +
...
# 
#   stbb <- ggplot(str_df, aes(x=bedbath)) +
#     geom_bar(fill=str_col, color=str_col) + 
#     xlab('\nBed/Bath Combo\n') +
#     ylab('Number of Properties\n') +
...
#  # Filter data
# 
#   ltr_df <- ltr_df[ltr_df$bedbath %in% acc.bb, ]
#   str_df <- str_df[str_df$bedbath %in% acc.bb, ]
# 
#  # Count Cleaning
#   
...
#   # Re-order factors
# 
#   str_df$bedbath <- factor(str_df$bedbath, 
#                               levels=c('2..2', '1..1', '2..1', '3..1', '3..2', '4..2'))
#   ltr_df$bedbath <- factor(ltr_df$bedbath, 
#                                levels=c('2..2', '1..1', '2..1', '3..1', '3..2', '4..2'))
# 
#   # Add product as a variable
#   
#   str_df$product <- paste0(substr(str_df$type, 1, 1), str_df$bedbath)
#   ltr_df$product <- paste0(substr(ltr_df$type, 1, 1), ltr_df$bedbath)
# 
# ```
# 
# 
...
# 
#   ltr_df <- ltr_df[!is.na(ltr_df$event.price), ]
