...
```{r source_files, message=FALSE, warning=FALSE, comment=FALSE, echo=TRUE, results='hide'}
 ...
 # Other custom files
  ...
  source(paste0(code.path, 'tools/spatialTools/spatialLogitModels.R'))
  source(paste0(code.path, 'tools/spatialTools/point2surface_Tools.R'))
...
```{r clip_time, message=FALSE, warning=FALSE, comment=FALSE, cache=FALSE, echo=TRUE}
 ...
 # Limit long term data  
  ...
  ltr.data <- ltr.data[ltr.data$event.date >= (strike_date - 365) & 
                            ltr.data$event.date <= (strike_date - 1), ]
...
```{r revdensplot, message=FALSE, warning=FALSE, comment=FALSE, cache=FALSE, echo=TRUE}
  ...
  ltr.data <- ltr.data[ltr.data$dom <= 180, ]
...
```{r convert_lt_subm, message=FALSE, warning=FALSE, comment=FALSE, cache=TRUE, echo=TRUE}
 ...
 ## Add the submarket designations
  ...
  # Add individual classes
  ltr.data$geo.mrkt <- 'rural'
  ltr.data$geo.mrkt[ltr.data$sub.class == 'Middle'] <- 'suburban'
  ltr.data$geo.mrkt[ltr.data$sub.class == 'Inner'] <- 'city'
  ltr.data$geo.mrkt[ltr.data$suburb %in% core.suburbs] <- 'city-core'
  ltr.data$geo.mrkt[ltr.data$beach == 1] <- 'beach'
  
  # Order the factors properly
  ltr.data$geo.mrkt <- factor(ltr.data$geo.mrkt, 
                              levels=c('city-core', 'city', 'suburban', 
                                        'rural', 'beach'))
...
```{r ltr.plot, message=FALSE, warning=FALSE, comment=FALSE, cache=TRUE, echo=TRUE}
  ...
  ltr.map <- ggplot() +
    geom_polygon(data=subs, aes(x=long, y=lat, group=group), fill='white', color='grey70') +
    geom_point(data=ltr.data, aes(x=longitude, y=latitude, color=geo.mrkt), size=.1,
               alpha=.1) +
    scale_color_manual(values=sm.col) +
    scale_x_continuous(limits=c(min(subs$long), max(subs$long))) +
    scale_y_continuous(limits=c(min(subs$lat), max(subs$lat))) +
...
```{r smtype.plot, message=FALSE, warning=FALSE, comment=FALSE, cache=TRUE, echo=TRUE, fig.width=9}
  ...
  ltrtype.map <- ggplot() +
    geom_bar(data=ltr.data, aes(x=geo.mrkt, fill=geo.mrkt)) +
    scale_fill_manual(values=sm.col) +
    facet_wrap(~type) +
    ylab('# of Properties\n') + xlab('')+
    theme(legend.position = 'bottom',
...
```{r message=FALSE, warning=FALSE, comment=FALSE, cache=TRUE, echo=TRUE}
  ...
  save(str_df, ltr.data, daily_df, listing.data, subs, abb.col, 
       ...
       file=paste0(data.path, 'prepared/prep_workspace_', data_name, '.RData'))
