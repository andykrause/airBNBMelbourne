...
```{r ltprice_fltr33s, message=FALSE, warning=FALSE, comment=FALSE, cache=FALSE, echo=TRUE}
  ...
  str.data <- str.data[str.data$med.rate >= 50 &
...
```{r convert_st_subm, message=FALSE, warning=FALSE, comment=FALSE, cache=TRUE, echo=TRUE}
 ...
 ## Add the submarket designations
  ...
  str.data$geo.mrkt <- 'rural'
  str.data$geo.mrkt[str.data$sub.class == 'Middle'] <- 'suburban'
  str.data$geo.mrkt[str.data$sub.class == 'Inner'] <- 'city'
  str.data$geo.mrkt[str.data$suburb %in% core.suburbs] <- 'city-core'
  str.data$geo.mrkt[str.data$beach == 1] <- 'beach'
  
  # Order the factors properly
  str.data$geo.mrkt <- factor(str.data$geo.mrkt, 
...
```{r locationmaps.plot, message=FALSE, warning=FALSE, comment=FALSE, cache=TRUE, echo=TRUE}
  ...
  str.map <- ggplot() +
    ...
    geom_point(data=str.data, aes(x=longitude, y=latitude, color=geo.mrkt), size=.2,
               alpha=.2) +
...
```{r smtype.plot, message=FALSE, warning=FALSE, comment=FALSE, cache=TRUE, echo=TRUE, fig.width=9}
  ...
  strtype.map <- ggplot() +
    geom_bar(data=str.data, aes(x=geo.mrkt, fill=geo.mrkt)) +
...
```{r redo.daily.summ, message=FALSE, warning=FALSE, comment=FALSE, cache=TRUE, echo=TRUE}
 ...
 cut.fields <- which(names(str.data) %in% c('total.days', 'block.rate', 'avail.rate',
                                     'longest.block', 'nbr.block', 'med.block', 'bookings'))
