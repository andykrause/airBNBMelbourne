...
subtitle:  "For the 'To Airbnb' paper"
...
This script executes a number of very long running data preparation process necessary to analyze the short term rental (Airbnb) data.  If you re-knit this file the code will not evaluate.  To actually evaluate the code you must manually re-run the code chunks individually in RStudio. 
...
```{r write_out}
 ...
 saveRDS(str_df, file=file.path(getwd(), 'data', 'prepared', 'str_data.RDS'))
