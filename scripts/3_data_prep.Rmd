...
```{r clip_time, message=FALSE, warning=FALSE, comment=FALSE, cache=FALSE, echo=TRUE}
  ...
  str_df <- str_df[(str_df$max_date - str_df$min_date) >= 178, ]
  str_df <- str_df[str_df$max_date >= (decision_date + 182) &
                    str_df$min_date <= (decision_date + 182), ]
  
  daily_df <- daily_df[daily_df$date >= decision_date, ]

  # Limit long term data

  ltr_df <- ltr_df[ltr_df$event_date >= (decision_date - 365) &
                    ltr_df$event_date <= (decision_date - 1), ]
  list_df <- list_df[list_df$id_key %in% ltr_df$id_key, ]
...
```{r add_month, message=FALSE, warning=FALSE, comment=FALSE, cache=TRUE, echo=TRUE}
  ...
  str_df$month <- 1
  daily_df$month <- paste0(lubridate::year(daily_df$date), "_", 
                           lubridate::month(daily_df$date))

  # Long term data
 
  ltr_df$month <- paste0(lubridate::year(ltr_df$event_date), "_",
                          lubridate::month(ltr_df$event_date))
