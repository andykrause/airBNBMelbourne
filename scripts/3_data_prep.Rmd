...
```{r bath_fltr, message=FALSE, warning=FALSE, comment=FALSE, cache=TRUE, echo=TRUE}
  ...
  str_tdf <- str_tdf %>% dplyr::filter(!is.na(bathrooms))
...
```{r bath_fltr2, message=FALSE, warning=FALSE, comment=FALSE, cache=FALSE, echo=TRUE, fig.height=5}
  ...
  # Build long term plot
   ...
   ltbath <- ggplot(ltr_tdf, aes(x=bathrooms)) + 
     geom_bar(fill=ltr_col, color=ltr_col) +
     facet_wrap(~type) +
 ...
 # Build short term plot
  ...
  stbath <- ggplot(str_tdf, aes(x=bathrooms)) +
    geom_bar(fill=str_col, color=str_col) +
    facet_wrap(~type, scales='free_y') +
...
```{r ltprice_fltr2, message=FALSE, warning=FALSE, comment=FALSE, cache=FALSE, echo=TRUE}
  ...
  ltr_price <- ggplot(ltr_tdf %>% tidyr::unnest(), aes(x=price)) +
  ...
  # Nightly Rate plot
  str_plot <- ggplot(str_tdf, aes(x=med_rate)) +
...
```{r revdensity.plot, message=FALSE, warning=FALSE, comment=FALSE, cache=FALSE, echo=FALSE, fig.width=6,fig.height=3.5, fig.cap='Comparison of Revenue Distributions'}
   ...
   makePotBookings <- function(x){
     round(nrow(x) * (length(which(x$status == 'R')) / 
                        length(which(x$status != 'B'))), 0)
   } 

   str_tdf <- str_tdf %>%
     mutate(bookings = unlist(map(daily_data, ~countBookings(.))),
            pot_bookings = unlist(map(daily_data, ~makePotBookings(.))),
            gross_revenue = bookings * med_rate,
            pot_revenue = pot_bookings * med_rate)
           
   # compute long term revenue
   ltr_tdf <- ltr_tdf %>%
...
```{r mappoints, message=FALSE, warning=FALSE, comment=FALSE, cache=TRUE, echo=TRUE}
  ...
  # Convert shapefile to ggplot-able object
   ...
   studysubs_sf <- sub_sf %>%
     filter(NAME_2006 %in% ltr_subs$suburb)
   
   # Convert to an outer bound with holes
   ss_bound <- st_union(studysubs_sf)
...
#    # Make map
#    ggplot(studysubs_sf) %>%
#      geom_sf(aes(group=group, fill=suburbDesi))
# 
#    loc_map <- ggplot() +
#      geom_polygon(data=studysubs_sf, 
#                   aes(x=long, y=lat, group=group, fill=des), alpha=.8) + 
...
```{r beachbuffer, message=FALSE, warning=FALSE, comment=FALSE, cache=TRUE, echo=TRUE}
 ...
 # Calculate Degress to Meters conversion 
  ...
  beach_buffer <- st_buffer(beach_sf, dist =(500 / m_conv))
...
```{r add_st_space, message=FALSE, warning=FALSE, comment=FALSE, cache=TRUE, echo=TRUE}
 ...
 # Convert to point shapefile
   str_tdf <- st_as_sf(str_tdf,
                       coords = c('longitude', 'latitude'),
                       crs = 4283)

   ltr_tdf <- st_as_sf(ltr_tdf %>% dplyr::filter(!is.na(latitude)),
                       coords = c('longitude', 'latitude'),
                       crs = 4283)

    str_tdf <- st_join(str_tdf, 
                       sub_sf %>% 
                        dplyr::select(suburb = NAME_2006,
                                      sub_des = suburbDesi,
                                      geometry)) %>%
    dplyr::filter(!is.na(sub_des))
    
    ltr_tdf <- st_join(ltr_tdf %>%
                         dplyr::select(-suburb), 
                       sub_sf %>% 
                        dplyr::select(suburb = NAME_2006,
                                      sub_des = suburbDesi,
   ...
   str_tdf <- st_join(str_tdf, 
                       beach_buffer %>%
                         dplyr::select(beach=id,
                                       geometry)) %>%
      mutate(beach = ifelse(is.na(beach), 0, beach))
...
```{r convert_st_subm, message=FALSE, warning=FALSE, comment=FALSE, cache=TRUE, echo=TRUE}
  ...
  ## Add the submarket designations
   ...
   str_tdf$geo_mrkt <- 'rural'
   str_tdf$geo_mrkt[str_tdf$sub_des == 'Middle'] <- 'suburban'
   str_tdf$geo_mrkt[str_tdf$sub_des == 'Inner'] <- 'city'
   str_tdf$geo_mrkt[str_tdf$suburb %in% core_suburbs] <- 'city-core'
   str_tdf$geo_mrkt[str_tdf$beach == 1] <- 'beach'
#   
#   # Order the factors properly
    str_tdf$geo_mrkt <- factor(str_tdf$geo_mrkt,
                               levels=c('city-core', 'city', 'suburban',
                                        'rural', 'beach'))
    
       # Add individual classes
   ltr_tdf$geo_mrkt <- 'rural'
   ltr_tdf$geo_mrkt[ltr_tdf$sub_des == 'Middle'] <- 'suburban'
   ltr_tdf$geo_mrkt[ltr_tdf$sub_des == 'Inner'] <- 'city'
   ltr_tdf$geo_mrkt[ltr_tdf$suburb %in% core_suburbs] <- 'city-core'
   ltr_tdf$geo_mrkt[ltr_tdf$beach == 1] <- 'beach'
#   
#   # Order the factors properly
   ltr_tdf$geo_mrkt <- factor(ltr_tdf$geo_mrkt,
                               levels=c('city-core', 'city', 'suburban',
                                        'rural', 'beach'))
 # Count Cleaning
