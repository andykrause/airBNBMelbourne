...
```{r source_files, message=FALSE, warning=FALSE, comment=FALSE, echo=TRUE, results='hide'}
  ...
  source(file.path(code_path, 'functions', 'geom_sf.R'))
...
```{r load_str_data, message=FALSE, warning=FALSE, comment=FALSE, cache=FALSE, echo=TRUE}
...
```{r new_study_area, message=FALSE, warning=FALSE, comment=FALSE, cache=FALSE, echo=TRUE}
  ...
  all_sf <- rbind(str_sf %>% 
                    rename(uniq_id = property_id),
                  ltr_sf %>%
                    rename(uniq_id = addr_id))
  ssub_sf <- sub_sf %>% filter(NAME_2006 %in% sub_all$suburb)

  all_ch <- st_buffer(st_convex_hull(st_union(ssub_sf)), .001)
  within_ch <- st_contains(all_ch, sub_sf$geometry)
  subs_within <- sub_sf[unlist(within_ch), ]
  study_area <- st_union(subs_within)
  plot(study_area)
...
```{r convert_st_subm, message=FALSE, warning=FALSE, comment=FALSE, cache=FALSE, echo=TRUE}
  ...
  ## Add to STR
   ...
   str_sf$geo_mrkt <- 'rural'
   str_sf$geo_mrkt[str_tdf$sub_des == 'Middle'] <- 'suburban'
   str_sf$geo_mrkt[str_tdf$sub_des == 'Inner'] <- 'city'
   str_sf$geo_mrkt[str_tdf$suburb %in% core_suburbs] <- 'city-core'
   str_sf$geo_mrkt[str_tdf$beach == 1] <- 'beach'
   ...
   # Add individual classes
   ltr_sf$geo_mrkt <- 'rural'
   ltr_sf$geo_mrkt[ltr_sf$sub_des == 'Middle'] <- 'suburban'
...
Finally we add the spatial data back onto the tabular data (filtering out those outside of the study area using an inner join).
...
```{r ltprice_fltr2, message=FALSE, warning=FALSE, comment=FALSE, cache=FALSE, echo=TRUE}
  ...
  # Nightly Rate plot
  str_price <- ggplot(str_tdf, aes(x=med_rate)) +
...
```
# 
# ```{r loctype.plot, message=FALSE, warning=FALSE, comment=FALSE, cache=FALSE, echo=TRUE, fig.width=9}
# 
