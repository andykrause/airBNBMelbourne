...
Data on AirBNB (short-term) rentals has been purchased from www.airdna.co, a data provider specializing in AirBNB data collection and analysis. The data on long-term leases was provided by Australia Property Monitors (APM), a Fairfax Group company. 

Throughout this document we will refer to the short-term data as **'short-term'** or **'AirBNB'** while the long-term data will be referred to as **'long-term'**, **'rental'** or **'lease'**.  Short-term nightly fees will be referenced as **'rates'** and long-term as **'rent'**.  We will use **'prices'** to collectively refer to **rates** and **rents**. Note that within the Melbourne market rentals are quoted in weekly rents (but are charged to tenants at a monthly rent equivalent to the 1/12 of 52 weeks of rent). 
...
```{r load_rent_data, message=FALSE, warning=FALSE, comment=FALSE, cache=TRUE, echo=TRUE}
 ...
 ## Property level long term rental information
  ...
  rent.data <- read.csv(paste0(data.path, '/prepared/ltpropdata.csv'), header=T)
...
```{r lower_names, message=FALSE, warning=FALSE, comment=FALSE, cache=TRUE, echo=TRUE}
 ...
 ## Long term data  
  ...
  names(rent.data) <- tolower(names(rent.data))
...
```{r trim_lt, message=FALSE, warning=FALSE, comment=FALSE, cache=TRUE, echo=TRUE}
 ...
 ## Long term property Data
  ...
  rent.keep.flds <- c('id.key', 'geographicalid', 'eventid', 'addressid', 'activityid',
  ...
  rent.data <- rent.data[ , rent.keep.flds]
...
```{r rename_lt, message=FALSE, warning=FALSE, comment=FALSE, cache=TRUE, echo=TRUE}
  ...
  names(rent.data) <- c('id.key', 'geographical.id', 
                        ...
                        'has.balcony', 'has.aircond', 'has.garage')
...
```{r fix_lt_time, message=FALSE, warning=FALSE, comment=FALSE, cache=TRUE, echo=TRUE}
 ...
 ## Convert event date  
  ...
  # Date
  rent.data$event.date <- fixLTDates(rent.data$event.date)

  # Year
  rent.data$event.year <- as.numeric(substr(rent.data$event.date, 1, 4))
...
```{r fix_lt_bools, message=FALSE, warning=FALSE, comment=FALSE, cache=TRUE, echo=TRUE}
  ...
  rent.data$has.study <- ifelse(rent.data$has.study == 'True', 1, 0)
  rent.data$has.courtyard <- ifelse(rent.data$has.courtyard == 'True', 1, 0)
  rent.data$has.aircon <- ifelse(rent.data$has.aircon == 'True', 1, 0)
  rent.data$has.balcony <- ifelse(rent.data$has.balcony == 'True', 1, 0)
  rent.data$has.garage <- ifelse(rent.data$has.garage == 'True', 1, 0)
...
```{r fix_type, message=FALSE, warning=FALSE, comment=FALSE, cache=TRUE, echo=TRUE}
 ...
 ## Long Term data  
  ...
  rent.data$type <- 'Other'
  rent.data$type[rent.data$property.type %in% c('Duplex', 'House',
                                           'Terrace', 'Townhouse')] <- 'House'
  rent.data$type[rent.data$property.type %in% c('Studio', 'Unit')] <- 'Apartment'
...
```{r remove_others, message=FALSE, warning=FALSE, comment=FALSE, cache=TRUE, echo=TRUE, fig.height=4}
  ...
  rent.data <- subset(rent.data, type != 'Other')
...
```{r clip_time, message=FALSE, warning=FALSE, comment=FALSE, cache=FALSE, echo=TRUE}
 ...
 ## Limit long term data  
  ...
  rent.dataf <- rent.data[rent.data$event.date >= '2014-09-01' & 
                            rent.data$event.date <= '2015-08-31', ]
  listing.dataf <- listing.data[listing.data$id.key %in% rent.dataf$id.key, ]
...
```{r mappoints, message=FALSE, warning=FALSE, comment=FALSE, cache=TRUE, echo=TRUE}
 ...
 ## Convert shapefile to ggplot-able object
  ...
  sub.table <- table(rent.dataf$suburb)
...
```{r fix_ltt_geo, message=FALSE, warning=FALSE, comment=FALSE, cache=TRUE, echo=TRUE}
 ...
 ## Label those missing lat/long  
  ...
  miss.LL <- which(is.na(rent.dataf$latitude))  

 ## Create a field to indicate the approximate matches  

  rent.dataf$miss.LL <- 0
  rent.dataf$miss.LL[miss.LL] <- 1

 ## Apply approximate match    

  rent.dataf$latitude[miss.LL] <- rent.dataf$street.latitude[miss.LL]
  rent.dataf$longitude[miss.LL] <- rent.dataf$street.longitude[miss.LL]

 ## Remove those still missing latitude data   

  rent.dataf <- rent.dataf[!is.na(rent.dataf$latitude), ]
...
```{r add_lt_space, message=FALSE, warning=FALSE, comment=FALSE, cache=TRUE, echo=TRUE}
 ...
 ## Long term data  
  ...
  # Convert to point shapefile
  rent.prop.shp <- SpatialPointsDataFrame(cbind(rent.dataf$longitude,
                                                rent.dataf$latitude),
                                         rent.dataf,
                                         proj4string=CRS("+init=epsg:4283")) 
  ...
  rent.dataf$suburb <- as.character(spJoin$name_2006)
  rent.dataf$suburb[is.na(rent.dataf$suburb)] <- 'missing'

  # Add suburb designation
  rent.dataf$sub.class <- as.character(spJoin$suburbdesi)
  rent.dataf$sub.class[is.na(rent.dataf$sub.class)] <- 'missing'
  
  # Add SA1 designation
  spJoin <- over(rent.prop.shp, sa1s.shp)
  rent.dataf$sa1 <- spJoin$sa1_main11
 
  ## Add Beach Designation
  spJoin <- over(rent.prop.shp, beach.buffer)
  rent.dataf$beach <- ifelse(is.na(spJoin), 0 ,1)
  
  # Remove those missing a suburb classification (out of area)
  rent.dataf <- rent.dataf[rent.dataf$sub.class != 'missing', ]
  rent.dataf <- rent.dataf[rent.dataf$sa1 != 'missing', ]
...
```{r convert_lt_subm, message=FALSE, warning=FALSE, comment=FALSE, cache=TRUE, echo=TRUE}
 ...
 ## Add the submarket designations
  ...
  # Add individual classes
  rent.dataf$sub.mrkt <- 'rural'
  rent.dataf$sub.mrkt[rent.dataf$sub.class == 'Middle'] <- 'suburban'
  rent.dataf$sub.mrkt[rent.dataf$sub.class == 'Inner'] <- 'city'
  rent.dataf$sub.mrkt[rent.dataf$suburb %in% core.suburbs] <- 'city-core'
  rent.dataf$sub.mrkt[rent.dataf$beach == 1] <- 'beach'
  
  # Order the factors properly
  rent.dataf$sub.mrkt <- factor(rent.dataf$sub.mrkt, 
                               levels=c('city-core', 'city', 'suburban', 
                                        'rural', 'beach'))
...
```{r rent.plot, message=FALSE, warning=FALSE, comment=FALSE, cache=TRUE, echo=TRUE}
  ...
  rent.map <- ggplot() +
    geom_polygon(data=subs, aes(x=long, y=lat, group=group), color='gray50') +
    geom_point(data=rent.dataf, aes(x=longitude, y=latitude, color=sub.mrkt), size=.1) +
    scale_color_manual(values=sm.col) +
    scale_x_continuous(limits=c(min(subs$long), max(subs$long))) +
    scale_y_continuous(limits=c(min(subs$lat), max(subs$lat))) +
    xlab('') + ylab('')+
...
```{r smtype.plot, message=FALSE, warning=FALSE, comment=FALSE, cache=TRUE, echo=TRUE, fig.width=9}
  ...
  # Make map
  renttype.map <- ggplot() +
    geom_bar(data=rent.dataf, aes(x=sub.mrkt, fill=sub.mrkt)) +
    scale_fill_manual(values=sm.col) +
    facet_wrap(~type) +
    ylab('# of Properties\n') + xlab('')+
    theme(legend.position = 'bottom',
...
```{r bed_fltr, message=FALSE, warning=FALSE, comment=FALSE, cache=TRUE, echo=TRUE}
 ...
 ## Remove missing and data errors
  ...
  # Long term data
  rent.dataf <- rent.dataf[!is.na(rent.dataf$bedrooms), ]
  rent.dataf <- rent.dataf[rent.dataf$bedrooms <= 10, ]
...
```{r bed_fltr2, message=FALSE, warning=FALSE, comment=FALSE, cache=FALSE, echo=TRUE, fig.height=5}
 ...
 ## Build long term bar plot
  ...
  rentbed <- ggplot(rent.dataf, aes(x=bedrooms)) + 
...
```{r bed_fltr3, message=FALSE, warning=FALSE, comment=FALSE, cache=TRUE, echo=TRUE}
 ...
 ## Remove all with more than 4 beds
  ...
  rent.dataf <- rent.dataf[rent.dataf$bedrooms < 5, ]

 ## Remove apartments with more than 3
  
  abba <- which(abb.dataf$bedrooms > 3 & abb.dataf$type =='Apartment')
  renta <- which(rent.dataf$bedrooms > 3 & rent.dataf$type =='Apartment')
  if(length(abba)>0) abb.dataf <- abb.dataf[-abba, ]
  if(length(renta)>0) rent.dataf <- rent.dataf[-renta, ]
...
```{r bath_fltr, message=FALSE, warning=FALSE, comment=FALSE, cache=TRUE, echo=TRUE}
 ...
 ## Remove missing data
rent.dataf <- rent.dataf[!is.na(rent.dataf$baths), ]
...
```{r bath_fltr2, message=FALSE, warning=FALSE, comment=FALSE, cache=FALSE, echo=TRUE, fig.height=5}
 ...
 ## Build long term plot
  ...
  ltbath <- ggplot(rent.dataf, aes(x=baths)) + 
    geom_bar(fill=abb.col[3], color=abb.col[3]) +
    facet_wrap(~type) +
...
```{r bath_fltr3, message=FALSE, warning=FALSE, comment=FALSE, cache=TRUE, echo=TRUE}
 ...
 ## Filter by bath count
  ...
  # Long term data
  rent.dataf <- rent.dataf[rent.dataf$baths < 4, ]
...
```{r filtBB, message=FALSE, warning=FALSE, comment=FALSE, cache=TRUE, echo=TRUE}
 ...
 ## Create combination variable
  ...
  rent.dataf$bedbath <- paste0(rent.dataf$bedrooms, '..', rent.dataf$baths)
...
```{r filtBB2, message=FALSE, warning=FALSE, comment=FALSE, cache=FALSE, echo=TRUE, fig.height=5}
 ...
 ## Make long term plot
  ...
  ltb <- ggplot(rent.dataf, aes(x=bedbath)) + 
    geom_bar(fill=abb.col[3], color=abb.col[3]) +
    xlab('\nBed/Bath Combo\n') +
...
```{r filtBB3, message=FALSE, warning=FALSE, comment=FALSE, cache=TRUE, echo=TRUE}
 ...
 ## Filter data
  ...
  rent.dataf <- rent.dataf[rent.dataf$bedbath %in% acc.bb, ]
...
```{r ltprice_fltr, message=FALSE, warning=FALSE, comment=FALSE, cache=TRUE, echo=TRUE}
  ...
  rent.dataf <- rent.dataf[!is.na(rent.dataf$event.price), ]
...
```{r ltprice_fltr2, message=FALSE, warning=FALSE, comment=FALSE, cache=FALSE, echo=TRUE}
  ...
  # Rent Plot
  rent.plot <- ggplot(rent.dataf, aes(x=event.price)) + 
...
```{r ltprice_fltr3, message=FALSE, warning=FALSE, comment=FALSE, cache=TRUE, echo=TRUE}
 ...
 ## Filter the long term data
  ...
  rent.dataf <- rent.dataf[rent.dataf$event.price >= 200 & 
                         rent.dataf$event.price <= 1000, ]
...
```{r filttransdata, message=FALSE, warning=FALSE, comment=FALSE, cache=TRUE, echo=TRUE}
 ...
 ## Long term list data
  ...
  listing.dataf <- listing.dataf[listing.dataf$activity.id %in% rent.dataf$activity.id,]
