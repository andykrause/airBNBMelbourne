...
This document describes all of the code used to the generate the output included in the 'Should I Airbnb my Property' study.  Additional analysis that were not included in the final report are also found within this document.  This analysis uses the output from the **abbDataClean_Script.rmd** file found in the same online repository. 
### Preliminary Commands

We begin by loading the libraries necessary to do this analysis. 
...
We then set the paths to the data, code and figures, based on the user of the document.  We also set a variable indicating whether or not we are working offline. 
...
Next, we source the necessary custom code files for the analysis.  We also set a few options here. 
...
```{r source_files, message=FALSE, warning=FALSE, comment=FALSE, echo=TRUE, results='hide'}
 ...
 # Other custom files
  ...
  source(paste0(code.path, '/spatialTools/spatialLogitModels.R'))
  source(paste0(code.path, '/spatialTools/point2surface_Tools.R'))
...
We load the workspace from the data clean script that includes the prepared data objects. 
...
```{r obs_revenues, message=FALSE, warning=FALSE, comment=FALSE, cache=TRUE, echo=TRUE}
  ...
  str.data$str.obs.revenue <- ((str.data$bookings * str.data$med.rate) - 
...
```{r act_revenues, message=FALSE, warning=FALSE, comment=FALSE, cache=TRUE, echo=TRUE}
  ...
  str.data$str.act.revenue <- ((str.data$bookings * str.data$extr.par *
                                  str.data$med.rate) - 
                                 (str.data$bookings * str.data$avg.guests * 
...
```{r pot_revenues, message=FALSE, warning=FALSE, comment=FALSE, cache=TRUE, echo=TRUE}
  ...
  str.data$str.pot.revenue <- ((str.data$pot.occ.rate * 366 * str.data$med.rate) - 
                                 (str.data$pot.occ.rate * 366 * 
                                    str.data$avg.guests * str.data$cpppd))
...
```{r message=FALSE, warning=FALSE, comment=FALSE, cache=TRUE, echo=TRUE}
  ...
  str.metro <- merge(str.data, imp.data$imp.rent, by='property.id')
...
```{r build.tbg.data, message=FALSE, warning=FALSE, comment=FALSE, cache=TRUE, echo=TRUE}
 ...
 str.data$tbg <- paste0(str.data$product,'.', str.data$geo.mrkt)
...
```{r plot_revdens, message=FALSE, warning=FALSE, comment=FALSE, cache=FALSE, echo=FALSE, fig.width=6,fig.height=3.5}
  ...
  # Actual
   stra.rev <- str.data.ic[,c('property.id', 'str.act.revenue')]
   stra.rev$tenure <- 'Short-Term (Actual)   '
   names(stra.rev)[2] <- 'revenue'
  
  # Potential
   strp.rev <- str.data.ic[,c('property.id', 'str.pot.revenue')]
   strp.rev$tenure <- 'Short-Term (Potential)   '
   names(strp.rev)[2] <- 'revenue'
  
  # Long-Term
   ltr.rev <- str.data.ic[,c('property.id', 'ltr.imp.revenue')]
...
```{r message=FALSE, warning=FALSE, comment=FALSE, cache=FALSE, echo=TRUE}
  ...
  table(str.data.ic$str.act.pref) / nrow(str.data.ic)
...
```{r message=FALSE, warning=FALSE, comment=FALSE, cache=FALSE, echo=FALSE}
 ...
 ## Set Preference data field
  ...
  str.data.ic$pref <- str.data.ic$str.act.pref
 ...
 ## Calculate raw and percentile occupancies
  ...
  rawocc.metro <- abbPrefPlot(str.data.ic,
                         x.field='occ.rate',
                         smooth=TRUE,
                         smooth.span=.5)
  
  qtlocc.metro <- abbPrefPlot(str.data.ic,
                        x.field='occ.qtl',
                        smooth=TRUE,
                        smooth.span=.5)
...
```{r message=FALSE, warning=FALSE, comment=FALSE, cache=TRUE, echo=TRUE}
 ...
 # Make Basic comparison table
  pref.table.type <- abbCreateCompTable(ic.df=str.data.ic,
                                   split.field='type')
...
```{r build_occ_geo, fig.width=8, fig.height=4, echo=FALSE} 
 ...
 ## Build data
  ...
  # Get IDs
  cc.id <- which(str.data.ic$geo.mrkt == 'city-core')
  city.id <- which(str.data.ic$geo.mrkt == 'city')
  sub.id <- which(str.data.ic$geo.mrkt == 'suburban')
  rural.id <- which(str.data.ic$geo.mrkt == 'rural')
  beach.id <- which(str.data.ic$geo.mrkt == 'beach')
...
```{r message=FALSE, warning=FALSE, comment=FALSE, cache=TRUE, echo=TRUE}
  ...
  str.data.ic$bedbath <- factor(str.data.ic$bedbath,
...
str.data.ic$blockpertime <- round(str.data.ic$nbr.block * str.data.ic$extr.par, 1)

str.data.ic$host.type <- 'Unknown'
str.data.ic$host.type[str.data.ic$blockpertime > 12] <- 'Multi-Platform User'
str.data.ic$host.type[str.data.ic$block.rate <= .25] <- 'Profit Seeker'
str.data.ic$host.type[str.data.ic$block.rate >= .75] <- 'Opportunistic Sharer'
str.data.ic$host.type <- factor(str.data.ic$host.type,
                             levels=c('Profit Seeker', 'Opportunistic Sharer', 
                                      'Multi-Platform User', 'Unknown'))

host.type <- ggplot(str.data.ic, 
...
host.type.bar <- ggplot(str.data.ic, aes(host.type, fill=host.type)) +
  geom_bar() +
...
save(clean.count, daily.data, listing.data, ltr.data, pref.table,
     ...
     s.subs, sssubs, str.data, str.data.ic, subs, all.mwl.coefs, all.mwl.surfs,
     core.suburbs, est.points, exch.rate, host.models, pref.models, prem.models,
     file=paste0(data.path, 'analyzed/analyzed_workspace.RData'))
