...
```{r message=FALSE, warning=FALSE, comment=FALSE, cache=TRUE, echo=TRUE}
  ...
  ltr.mod.spec <- formula(log(event.price) ~ as.factor(type) + 
                            ...
                            as.factor(ltr.month))
  str.mod.spec <- formula(log(nightly.rate) ~ as.factor(type) + 
                            as.factor(bedbath) + as.factor(suburb))
...
```{r message=FALSE, warning=FALSE, comment=FALSE, cache=TRUE, echo=TRUE}
  ...
  str.imp <- imp.data$str
  ltr.imp <- imp.data$ltr
  
  # Extract models
  str.sum <- summary(imp.data$str.mod)
...
```{r message=FALSE, warning=FALSE, comment=FALSE, cache=TRUE, echo=TRUE}
  ...
  # Apply dom to abb
  str.imp$imp.dom <- imputeDOM(str.imp, ltr.imp, calc.type='median')
  
  # Apply occ.rate to ltr
  ltr.imp$imp.occ <- imputeOccRate(ltr.imp, str.imp, calc.type='median')
