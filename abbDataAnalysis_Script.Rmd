...
```{r message=FALSE, warning=FALSE, comment=FALSE, cache=TRUE, echo=TRUE}
  ...
  ss60 <- which(str.data.ic$cancel.policy == 'Super Strict 60 Days')
  str.data.ic$cancel.policy[ss60] <- 'Strict'
  cpmiss <- which(str.data.ic$cancel.policy == '')
  str.data.ic <- str.data.ic[-cpmiss, ]
  ...
  mod.str <- glm(str.pot.pref~type+as.factor(bedbath)+
                    sub.mrkt+
