...
```{r message=FALSE, warning=FALSE, comment=FALSE, cache=TRUE, echo=TRUE}
  ...
  str.data$pot.occ.rate <- (str.data$bookings * str.data$extr.par) / (366 - (366 * str.data$block.rate)) 
...
abbImputeCompare <- function(str.df,
  ...
  for(il in 1:length(str.list)){
    ...
    str.list[[il]]$imp.dom <- imputeDOM(str.list[[il]], 
                                        ltr.data[[il]], 
                                        calc.type='median')
    str.list[[il]]$ltr.imp.revenue <- str.list[[il]]$imp.rent * (52 - str.list[[il]]$imp.dom / 7)
...
```{r message=FALSE, warning=FALSE, comment=FALSE, cache=TRUE, echo=TRUE}
  ...
  mrkt.table <- createCompTable(str.glob, 'sub.mrkt')
...
```{r message=FALSE, warning=FALSE, comment=FALSE, cache=TRUE, echo=TRUE}
  ...
  rawocc.pplot <- makePrefPlot(str.glob,
                               x.field='pot.occ.rate',
                               y.field='str.pot.pref',
...
```{r message=FALSE, warning=FALSE, comment=FALSE, cache=TRUE, echo=TRUE}
  ...
  str.glob$sub.mrkt <- factor(str.glob$sub.mrkt, 
                         levels=c('city-core', 'city', 'suburban', 'rural', 'beach'))
  ggplot(str.glob, aes(x=pot.occ.rate, group=sub.mrkt, color=sub.mrkt, fill=sub.mrkt)) +
...
```{r message=FALSE, warning=FALSE, comment=FALSE, cache=TRUE, echo=TRUE}
  ...
  str.glob$occ.qtl <- makeWtdQtl(str.glob$occ.rate, 
                                      return.type='rank') 
  str.glob$rate.qtl <- makeWtdQtl(str.glob$med.rate, 
                                  return.type='rank') 
  
  ## Make quartile location plot
  
  qtlocc.pplot <- makePrefPlot(str.glob,
                               x.field='occ.qtl',
...
```{r message=FALSE, warning=FALSE, comment=FALSE, cache=TRUE, echo=TRUE}
  ...
  rate.hm <- makeHeatMap(str.glob,
                x.field='occ.rate',
                y.field='nightly.rate',
                color.field='str.act.pref',
                bins=c(.05, 25),
                svm=F, 
                alpha.count=T,
                add.points=T,
...
```{r message=FALSE, warning=FALSE, comment=FALSE, cache=TRUE, echo=TRUE}
  ...
  rate.hm.svm <- makeHeatMap(str.glob,
                         x.field='occ.rate',
                         y.field='nightly.rate',
                         color.field='str.act.pref',
                         bins=c(.05, 25),
...
```{r message=FALSE, warning=FALSE, comment=FALSE, cache=TRUE, echo=TRUE}
  ...
  qtl.hm.svm <- makeHeatMap(str.glob,
                        x.field='occ.qtl',
                        y.field='rate.qtl',
                        color.field='str.act.pref',
                        bins=c(5, 5),
...
```{r message=FALSE, warning=FALSE, comment=FALSE, cache=TRUE, echo=TRUE}
  ...
  market.ratio <- data.frame(type=c('rate', 'qtl'),
                             actual=rep(mean(str.glob$str.act.pref), 2),
