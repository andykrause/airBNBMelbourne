...
### Working analysis ---------------------------------------------------------------------  
 ...
 ## Impute globally 
  ...
  glob.imp <- crossImputeModel(rent.df=rent.dataf, 
                               abb.df=abb.dataf, 
 ...
 ## Calculate market occ and rate quantiles
  ...
  # Split into markets
  abb.mrkt <- split(glob.imp$abb, glob.imp$abb$product)
  rent.mrkt <- split(glob.imp$rent, glob.imp$rent$product)
  
  # Calculate AirBNB Quantiles
  abb.qtls <- lapply(abb.mrkt, calcOPQuantiles, daily.df=daily.dataf)
  rent.qtls <- lapply(rent.mrkt, calcLTQuantiles)  
  
 ## Calculate the revenues
  
  rev.list <- mapply(calcRevenues, 
                     abb.qtls=abb.qtls, 
                     rent.qtls=rent.qtls,
                     SIMPLIFY=FALSE,
                     MoreArgs=list(mrkt.occ=TRUE,
                                   mrkt='sub.mrkt'))
  all.revs <- rbind.fill(rev.list)
  
  
  

  
## Suburb specific occ.rates
  
  aa <- rbind.fill(lapply(abb.qtls, function(x) x$abb.df))
